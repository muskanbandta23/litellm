...
class AutoRouterBenchmarkTotals(BaseModel):
    ...
    spend: float | None = Field(
        description="Actual auto-router cost; unavailable in daily totals because daily rows mix router and other "
        "traffic. Per-router groups report tracked session spend. Zero for an entirely empty window"
    )
    classifier_cost: float | None = Field(
        description="Recorded classifier cost included in spend; unavailable in daily totals. Session groups require "
        "every turn to record the subtotal. Zero for an entirely empty window"
    )
    savings_estimated_turns: int = Field(
        description="Tracked session turns covered by the current savings estimator; legacy estimates are excluded "
        "from this coverage count, not from recorded savings"
    )
    ...
    saved_pct: float | None = Field(
        description="Recorded savings over actual spend plus recorded savings; unavailable for daily totals "
        "without matching actual cost"
    )
